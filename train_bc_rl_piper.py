"""
Piper BC 预训练 + RL 微调训练脚本
先使用示范数据进行行为克隆，然后用 RL 微调
"""
...
def load_bc_checkpoints(agent, bc_checkpoint_path, device):
    ...
    if not os.path.exists(bc_checkpoint_path):
        print(f"⚠️ BC checkpoint 不存在: {bc_checkpoint_path}")
        return False
...
def load_demo_data(demo_dir="./demo_data"):
    ...
    if not os.path.exists(demo_dir):
        print(f"⚠️ 示范数据目录不存在: {demo_dir}")
        return 0
    
    demo_files = glob.glob(os.path.join(demo_dir, "*.pkl"))
    if len(demo_files) == 0:
        print(f"⚠️ 在 {demo_dir} 中没有找到示范数据")
        return 0
    
    print(f"找到 {len(demo_files)} 个示范文件")
    
    # 注意：这里需要根据实际的 replay buffer 格式调整
    # 简化实现，只返回数量
    total_transitions = 0
    ...
    print(f"✓ 共 {total_transitions} 个示范转换")
...
class Workspace:
    def __init__(self, cfg):
        ...
        # BC 预训练配置
        self.bc_checkpoint_path = getattr(cfg, 'bc_checkpoint_path', None)
        self.load_demo_to_buffer = getattr(cfg, 'load_demo_to_buffer', False)
        self.demo_dir = getattr(cfg, 'demo_dir', './demo_data')
        ...
        self._nstep_beta = cfg.nstep_beta
        self._nstep_beta_temp = cfg.nstep_beta_temp
        self.setup()
        self.agent = make_agent(self.train_env.observation_spec(),
                                self.train_env.action_spec(), self.cfg.agent)
        
        # 如果指定了 BC checkpoint，加载预训练模型
        if self.bc_checkpoint_path is not None:
            success = load_bc_checkpoints(self.agent, self.bc_checkpoint_path, self.device)
            if success:
                print("✓ BC 预训练权重已加载")
            else:
                print("⚠️ BC 预训练权重加载失败，从随机初始化开始")
        
        # 加载示范数据到 buffer
        if self.load_demo_to_buffer:
            num_demos = load_demo_data(self.demo_dir)
            print(f"✓ 已准备 {num_demos} 个示范转换用于训练")
    ...
    def train(self):
        ...
        metrics = None
        print("start training (with BC initialization)")
...
def main(cfgs):
    from train_bc_rl_piper import Workspace as W
    root_dir = Path.cwd()
    workspace = W(cfgs)
    ...
    if hasattr(cfgs, 'snapshot_path') and cfgs.snapshot_path:
        ...
        snapshot_path = str(root_dir / 'snapshots' / f'snapshot_{cfgs.load_id}.pt')
    else:
        snapshot_path = str(root_dir / 'snapshot.pt')
